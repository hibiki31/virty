/*
VirtyAPI

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 4.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"fmt"
)

// checks if the NodeRole type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &NodeRole{}

// NodeRole struct for NodeRole
type NodeRole struct {
	RoleName string `json:"roleName"`
	ExtraJson map[string]interface{} `json:"extraJson,omitempty"`
}

type _NodeRole NodeRole

// NewNodeRole instantiates a new NodeRole object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewNodeRole(roleName string) *NodeRole {
	this := NodeRole{}
	this.RoleName = roleName
	return &this
}

// NewNodeRoleWithDefaults instantiates a new NodeRole object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewNodeRoleWithDefaults() *NodeRole {
	this := NodeRole{}
	return &this
}

// GetRoleName returns the RoleName field value
func (o *NodeRole) GetRoleName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.RoleName
}

// GetRoleNameOk returns a tuple with the RoleName field value
// and a boolean to check if the value has been set.
func (o *NodeRole) GetRoleNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RoleName, true
}

// SetRoleName sets field value
func (o *NodeRole) SetRoleName(v string) {
	o.RoleName = v
}

// GetExtraJson returns the ExtraJson field value if set, zero value otherwise.
func (o *NodeRole) GetExtraJson() map[string]interface{} {
	if o == nil || IsNil(o.ExtraJson) {
		var ret map[string]interface{}
		return ret
	}
	return o.ExtraJson
}

// GetExtraJsonOk returns a tuple with the ExtraJson field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *NodeRole) GetExtraJsonOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.ExtraJson) {
		return map[string]interface{}{}, false
	}
	return o.ExtraJson, true
}

// HasExtraJson returns a boolean if a field has been set.
func (o *NodeRole) HasExtraJson() bool {
	if o != nil && !IsNil(o.ExtraJson) {
		return true
	}

	return false
}

// SetExtraJson gets a reference to the given map[string]interface{} and assigns it to the ExtraJson field.
func (o *NodeRole) SetExtraJson(v map[string]interface{}) {
	o.ExtraJson = v
}

func (o NodeRole) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o NodeRole) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["roleName"] = o.RoleName
	if !IsNil(o.ExtraJson) {
		toSerialize["extraJson"] = o.ExtraJson
	}
	return toSerialize, nil
}

func (o *NodeRole) UnmarshalJSON(bytes []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"roleName",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(bytes, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varNodeRole := _NodeRole{}

	err = json.Unmarshal(bytes, &varNodeRole)

	if err != nil {
		return err
	}

	*o = NodeRole(varNodeRole)

	return err
}

type NullableNodeRole struct {
	value *NodeRole
	isSet bool
}

func (v NullableNodeRole) Get() *NodeRole {
	return v.value
}

func (v *NullableNodeRole) Set(val *NodeRole) {
	v.value = val
	v.isSet = true
}

func (v NullableNodeRole) IsSet() bool {
	return v.isSet
}

func (v *NullableNodeRole) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNodeRole(val *NodeRole) *NullableNodeRole {
	return &NullableNodeRole{value: val, isSet: true}
}

func (v NullableNodeRole) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNodeRole) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


