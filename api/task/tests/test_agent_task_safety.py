...
def test_target_reservation_rolls_back_partial_multi_target_acquisition(
    ...
    with db_factory() as db:
        ...
        acquire_target_reservations(
            db,
            resolved_targets=[busy_target],
            correlation_id=blocker.correlation_id,
            lease_id=blocker.lease_id,
        )
        db.commit()
    ...
    with db_factory() as db:
        ...
        with pytest.raises(TaskTargetBusy):
            acquire_target_reservations(
                ...
                lease_id=refresh.lease_id,
            )

        first.status = "finish"
        second.status = "finish"
        db.flush()
        assert release_target_reservations_if_terminal(db, first.correlation_id)
        assert release_target_reservations_if_terminal(db, second.correlation_id)
        ...
        with pytest.raises(TaskTargetBusy):
            acquire_target_reservations(
                db,
                resolved_targets=third.resolved_targets,
    ...
    with db_factory() as db:
        ...
        with pytest.raises(TaskTargetBusy):
            acquire_target_reservations(
                db,
                resolved_targets=exclusive_target,
                correlation_id=writer.correlation_id,
        ...
        with pytest.raises(TaskTargetBusy):
            acquire_target_reservations(
                db,
                resolved_targets=shared_target,
                correlation_id=blocked_reader.correlation_id,
                lease_id=blocked_reader.lease_id,
            )
...
def test_node_lifecycle_writer_conflicts_only_on_same_node(
    ...
    with db_factory() as db:
        ...
        acquire_target_reservations(
            db,
            resolved_targets=other_node_target,
            correlation_id=other_node_writer.correlation_id,
            lease_id=other_node_writer.lease_id,
        )
        ...
        with pytest.raises(TaskTargetBusy):
            acquire_target_reservations(
                db,
                resolved_targets=same_node_writer_target,
                correlation_id=same_node_writer.correlation_id,
                lease_id=same_node_writer.lease_id,
            )
...
def test_terminal_reservation_can_be_taken_over_but_orphan_cannot(
    ...
    with db_factory() as db:
        ...
        assert reservation.correlation_id == second.correlation_id
...
def test_unknown_operation_keeps_reservation_until_reconciled(
    ...
    with db_factory() as db:
        ...
        acquire_target_reservations(
            db,
            resolved_targets=AGENT_TARGET,
            correlation_id=uncertain.correlation_id,
            lease_id=uncertain.lease_id,
        )
...
def test_target_reservation_releases_only_after_entire_chain_is_terminal(
    ...
    with db_factory() as db:
        ...
        acquire_target_reservations(
            db,
            resolved_targets=root.resolved_targets,
        ...
        assert release_target_reservations_if_terminal(db, root.correlation_id) is False
        assert db.query(TaskTargetReservationModel).count() == 2

        child.status = "cancelled"
        db.flush()
        assert release_target_reservations_if_terminal(db, root.correlation_id) is True
        assert db.query(TaskTargetReservationModel).count() == 0
...
def test_soft_archive_preserves_agent_ledger_and_active_reservations(
    ...
    with db_factory() as db:
        ...
        acquire_target_reservations(
            db,
            resolved_targets=agent_terminal.resolved_targets,
            correlation_id=agent_terminal.correlation_id,
            lease_id=agent_terminal.lease_id,
        )
...
def test_operation_cancellation_can_share_outer_audit_transaction(
    ...
    with db_factory() as db:
        ...
        persisted = db.get(TaskModel, root.uuid)
...
def test_agent_policy_is_fail_closed_when_validator_is_missing(
    ...
    with db_factory() as db:
        ...
        acquire_target_reservations(
            db,
            resolved_targets=agent.resolved_targets,
...
def test_task_runner_commits_handler_database_updates(
    ...
    with db_factory() as db:
        persisted = db.get(TaskModel, task_uuid)
...
def test_task_runner_rechecks_policy_while_target_lock_is_held(
    ...
    with db_factory() as db:
        ...
        acquire_target_reservations(
            db,
            resolved_targets=task.resolved_targets,
            correlation_id=task.correlation_id,
...
def test_worker_holds_reservation_when_outcome_audit_fails(
    ...
    with db_factory() as db:
        uncertain = db.get(TaskModel, task_uuid)
...
def test_worker_marks_cancelled_when_request_arrives_during_lock_wait(
    ...
    def release_on_terminal(db: Session, task: TaskModel) -> None:
        release_target_reservations_if_terminal(db, task.correlation_id)
    ...
    with db_factory() as db:
        cancelled = db.get(TaskModel, task_uuid)
        assert handler_called is False
...
def test_worker_marks_agent_handler_failure_after_effect_unknown(
    ...
    with db_factory() as db:
        unknown = db.get(TaskModel, task_uuid)
...
def test_worker_releases_reservation_only_after_success_commit_ack(
    ...
    with db_factory() as db:
        assert db.get(TaskModel, task_uuid).status == "finish"
        assert db.query(TaskTargetReservationModel).count() == 2
...
def test_worker_commit_ambiguity_keeps_unknown_reservation(
    ...
    with db_factory() as db:
        task = db.get(TaskModel, task_uuid)
...
def test_worker_restart_preserves_legacy_lost_semantics_and_agent_queue(
    ...
    with db_factory() as db:
        legacy_queued = db.get(TaskModel, task_ids[0])
        legacy_started = db.get(TaskModel, task_ids[1])
        agent_queued = db.get(TaskModel, task_ids[2])
...
def test_worker_stops_serial_dependency_chain_after_unfinished_task(
    ...
    with db_factory() as db:
        stopped = db.get(TaskModel, second_uuid)
...
def test_worker_resumes_serial_dependency_after_reconciliation(
    ...
    worker.run_scheduler(TaskBase())
    with db_factory() as db:
        assert db.get(TaskModel, dependent_uuid).status == "init"
