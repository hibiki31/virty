...
TEST_DATABASE_URL_ENV = "TASK_TARGET_LOCK_TEST_DATABASE_URL"
...
def test_postgresql_target_lock_supports_family_reader_writer() -> None:
    database_url = os.getenv(TEST_DATABASE_URL_ENV)
    if not database_url:
        pytest.skip(f"{TEST_DATABASE_URL_ENV}が未設定です")
...
def test_postgresql_node_lifecycle_lock_is_node_scoped() -> None:
    database_url = os.getenv(TEST_DATABASE_URL_ENV)
    if not database_url:
        pytest.skip(f"{TEST_DATABASE_URL_ENV}が未設定です")
...
def test_postgresql_reservations_support_family_reader_writer() -> None:
    database_url = os.getenv(TEST_DATABASE_URL_ENV)
    if not database_url:
        pytest.skip(f"{TEST_DATABASE_URL_ENV}が未設定です")
    ...
    family_target = _family_task("pg-family-target").resolved_targets
...
def test_postgresql_reservation_unique_race_is_fail_closed() -> None:
    database_url = os.getenv(TEST_DATABASE_URL_ENV)
    if not database_url:
        pytest.skip(f"{TEST_DATABASE_URL_ENV}が未設定です")
    ...
    try:
        ...
        with factory.begin() as db:
            winner = db.get(TaskModel, "pg-race-first")
    ...
    def submit() -> None:
        try:
            with factory() as db:
                ...
                manager._find_idempotent_task = synchronize_first_lookup
                task = manager.commit(
...
def test_postgresql_user_delete_preserves_agent_idempotency_ledger(
    ...
    try:
        ...
        with factory() as db:
            preserved = db.get(TaskModel, task_uuid)
