...
def test_project_crud_task_status_path_and_validation(
    ...
    try:
        ...
        secret_name = "DoNotReturnThisMemberFilter" + ("x" * 256)
        invalid_candidates = api_client.get(
            f"/api/projects/{project_id}/member-candidates",
            headers=member_headers,
            params={"limit": 1001, "nameLike": secret_name},
        )
        assert invalid_candidates.status_code == 422
        assert invalid_candidates.json() == {
            "detail": {
        ...
        assert secret_name not in invalid_candidates.text
        assert '"input"' not in invalid_candidates.text
        assert '"ctx"' not in invalid_candidates.text
        assert '"msg"' not in invalid_candidates.text
        ...
        assert last_member.json()["detail"]["code"] == "project_member_conflict"
        ...
        assert guarded_delete.json()["detail"]["code"] == "project_not_empty"
...
def test_flavor_create_list_delete_path_and_validation(
    ...
    try:
        ...
        assert missing_delete.json()["detail"]["code"] == "flavor_not_found"
