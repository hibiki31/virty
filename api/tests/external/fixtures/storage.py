import pytest
from fastapi.testclient import TestClient

from module.ansiblelib import AnsibleManager
from storage.schemas import StoragePage
from tests.external.conftest import EnvConfig, wait_tasks


def _exact_storages(page: StoragePage, name: str, node_name: str):
    return [
        storage
        for storage in page.data
        if storage.name == name and storage.node_name == node_name
    ]


@pytest.fixture(scope="function")
def created_storage(env, client, nodes):
    create_storage(env, client)

    res = client.get("/api/storages")
    try:
        yield StoragePage.model_validate(res.json())
    finally:
        delete_storage(env, client, skipp=True)
        delete_storage_paths(env)


@pytest.fixture(scope="function")
def deleted_storage(env, client, nodes):
    delete_storage(env, client, skipp=True)
    delete_storage_paths(env)

    res = client.get("/api/storages")
    try:
        yield StoragePage.model_validate(res.json())
    finally:
        delete_storage(env, client, skipp=True)
        delete_storage_paths(env)


def create_storage(env: EnvConfig, client: TestClient):
    reload_storage(env, client)
    
    for server in env.servers:
        for storage in env.storages:  
            req_data = {
                "name": storage.name,
                "nodeName": server.name,
                "path": storage.path
            }
            
            # すでにあるか判定
            res_storage = client.get("/api/storages", params={"nameLike": storage.name, "nodeName": server.name})
            page = StoragePage.model_validate(res_storage.json())
            exact_storages = _exact_storages(page, storage.name, server.name)
            assert not exact_storages, (
                f"既存storageは再利用できません: {server.name}/{storage.name}"
            )
            
            res = client.post('/api/tasks/storages', json=req_data)
            
            assert res.status_code == 200
            assert wait_tasks(res, client) == "finish"
    
    reload_storage(env, client)
            
            
def delete_storage(env: EnvConfig, client: TestClient, skipp=False):
    reload_storage(env, client)
    for server in env.servers:
        for storage in env.storages:  
            res_storage = client.get("/api/storages", params={"nameLike": storage.name, "nodeName": server.name})
            
            page = StoragePage.model_validate(res_storage.json())
            exact_storages = _exact_storages(page, storage.name, server.name)
            if not exact_storages and skipp:
                continue
            
            assert len(exact_storages) == 1
            pool_uuid = exact_storages[0].uuid
            
            res = client.delete(f'/api/tasks/storages/{pool_uuid}')
            
            assert res.status_code == 200
            assert wait_tasks(res, client) == "finish"
    reload_storage(env, client)
            
            
def reload_storage(env: EnvConfig, client: TestClient):
    put_image = client.put("/api/tasks/images", params={})
    wait_tasks(put_image, client)


def delete_storage_paths(env: EnvConfig) -> None:
    """各test後に、このrun専用storage pathの内容も管理nodeから削除する。"""

    for server in env.servers:
        manager = AnsibleManager(user=server.username, domain=server.domain)
        for storage in env.storages:
            manager.run(
                playbook_name="pb_deleteinnode",
                extravars={
                    "host": f"{server.username}@{server.domain}",
                    "file": storage.path,
                },
                timeout=120,
            )
